...
for index, row in all_data.iterrows():
    if row['all_counts'] == 0:
        ...
        a = (row['p_counts'] / row['all_counts']) * \
            (1 + beta1 * (row['gender'] - s0) + beta2 * (row['age'] - a0) + beta3 * (row['temperature'] - t0))
    Index.append(a)
...
fin_data.to_csv(r'D:\python数据分析\2023TaiDi-A-master\middle\bingduindex.csv')
print("完成病毒系数计算")
print(result)
print('result↑')
# 查找疫苗信息
...
print(result5)
print('result↑555555555555')
all_data = pd.merge(fin_data, result5, on='user_id', how='left')
all_data.fillna(0, inplace=True)
print(all_data)
all_data.to_csv(r'D:\python数据分析\2023TaiDi-A-master\middle\bingduindex.csv')
...
# 设置图表标题、坐标轴标签和图例
ax.set_title('不同病毒传播指数下疫苗人数占比')
ax.set_xlabel('病毒感传播指数')
ax.set_ylabel('占比')
plt.xticks(list1, list1)
plt.tight_layout()
plt.savefig(r'./image/不同病毒传播指数下疫苗人数占比.png')
plt.show()
