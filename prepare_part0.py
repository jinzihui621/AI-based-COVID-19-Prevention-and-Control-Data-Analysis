...
conn = connect = pymssql.connect(host='localhost', port='1433', user='jiang', password='123456',
                                 database='covid-19control')
engine = create_engine('mssql+pymssql://jiang:123456@127.0.0.1/covid-19control')

db = conn.cursor()  # 获取游标

data2 = pd.read_csv(r'E:\jiang\pythonA\data\附件2.csv')
...
data3 = pd.read_csv(r'E:\jiang\pythonA\data\附件3.csv')
...
data4 = pd.read_csv(r'E:\jiang\pythonA\data\附件4.csv')
...
data6 = pd.read_csv(r'E:\jiang\pythonA\data\附件6.csv')
...
print("完成附件6")
data7 = pd.read_csv(r'E:\jiang\pythonA\data\附件7.csv')
...
path = r'E:\jiang\pythonA\data\附件5'
columns = [
    ('sno', 'int'),
    ('grid_point_id', 'int'),
    ('user_id', 'int'),
...
for filename in os.listdir(path):
    if filename.endswith('.csv') and len(filename) == 12:
        ...
        # 插入数据
        df.to_sql('场所码扫码信息表', engine, if_exists='append', index=False)
        conn.commit()
        a += 1
